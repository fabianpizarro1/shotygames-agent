...
// ════════════════════════════════════════════════════════════
// CALENDAR helpers (server-side via HTTPS)
// ════════════════════════════════════════════════════════════
let _calTok = null, _calTokTs = 0, _calList = null, _calListTs = 0;

async function getCalTok() {
  if (_calTok && Date.now() - _calTokTs < 50 * 60 * 1000) return _calTok;
  const body = `client_id=${process.env.GOOGLE_CLIENT_ID}&client_secret=${process.env.GOOGLE_CLIENT_SECRET}&refresh_token=${process.env.GOOGLE_CALENDAR_TOKEN || process.env.GOOGLE_REFRESH_TOKEN}&grant_type=refresh_token`;
  const data = await new Promise((res, rej) => {
    const r = https.request({ hostname: 'oauth2.googleapis.com', path: '/token', method: 'POST', headers: { 'Content-Type': 'application/x-www-form-urlencoded', 'Content-Length': Buffer.byteLength(body) } }, rs => {
      let d = ''; rs.on('data', c => d += c); rs.on('end', () => res(JSON.parse(d)));
    }); r.on('error', rej); r.write(body); r.end();
  });
  _calTok = data.access_token; _calTokTs = Date.now();
  return _calTok;
}

async function calGet(path) {
  const tok = await getCalTok();
  return new Promise((res, rej) => {
    const r = https.request({ hostname: 'www.googleapis.com', path, headers: { Authorization: 'Bearer ' + tok } }, rs => {
      let d = ''; rs.on('data', c => d += c); rs.on('end', () => res(JSON.parse(d)));
    }); r.on('error', rej); r.end();
  });
}

async function getCalList() {
  if (_calList && Date.now() - _calListTs < 60 * 60 * 1000) return _calList;
  const data = await calGet('/calendar/v3/users/me/calendarList');
  _calList = (data.items || []).map(c => ({ id: c.id, nombre: c.summary }));
  _calListTs = Date.now();
  return _calList;
}

async function getEventosHoy() {
  const cals = await getCalList();
  const fechaLocal = new Date().toLocaleDateString('en-CA', { timeZone: 'America/Guayaquil' });
  const qs = `timeMin=${encodeURIComponent(fechaLocal + 'T00:00:00-05:00')}&timeMax=${encodeURIComponent(fechaLocal + 'T23:59:59-05:00')}&singleEvents=true&orderBy=startTime&maxResults=30`;
  let todos = [];
  for (const cal of cals) {
    if (cal.nombre === 'Comidas') continue;
    try {
      const data = await calGet(`/calendar/v3/calendars/${encodeURIComponent(cal.id)}/events?${qs}`);
      (data.items || []).forEach(e => todos.push({
        id: e.id, titulo: e.summary || '(sin título)', calendario: cal.nombre,
        inicio: e.start?.dateTime || e.start?.date, todo_el_dia: !e.start?.dateTime
      }));
    } catch (_) {}
  }
  return todos.sort((a, b) => new Date(a.inicio) - new Date(b.inicio));
}

// ════════════════════════════════════════════════════════════
// BOT PERSONAL — 10am — Briefing diario + recordatorios
// ════════════════════════════════════════════════════════════
function programarRecordatorios(eventos) {
  const ahora = Date.now();
  for (const e of eventos) {
    if (e.todo_el_dia) continue;
    const ms = new Date(e.inicio).getTime() - 5 * 60000 - ahora;
    if (ms <= 0) continue;
    setTimeout(async () => {
      try {
        const hora = new Date(e.inicio).toLocaleTimeString('es-EC', { hour: '2-digit', minute: '2-digit', timeZone: 'America/Guayaquil' });
        await tg('personal', `⏰ <b>EN 5 MIN: ${e.titulo}</b>\n🕐 ${hora}`);
      } catch (_) {}
    }, ms);
  }
}

async function enviarBriefingPersonal() {
  if (!TOKENS.personal || !CHAT) return;
  try {
    const { listarTareas } = require('./sheets-personal');
    const [eventos, tareas] = await Promise.all([getEventosHoy(), listarTareas()]);
    const pendientes = (tareas || []).filter(t => (t.estado || '').toUpperCase() !== 'COMPLETADA');

    const fecha = new Date().toLocaleDateString('es-ES', { weekday: 'long', day: 'numeric', month: 'long', timeZone: 'America/Guayaquil' });
    let msg = `🌅 <b>BUENOS DÍAS — ${fecha.toUpperCase()}</b>\n\n`;

    if (eventos.length > 0) {
      msg += `<b>📅 AGENDA:</b>\n`;
      eventos.forEach(e => {
        if (e.todo_el_dia) return;
        const h = new Date(e.inicio).toLocaleTimeString('es-EC', { hour: '2-digit', minute: '2-digit', timeZone: 'America/Guayaquil' });
        msg += `🕐 ${h} — ${e.titulo}\n`;
      });
      msg += '\n';
    }
    ...
    if (pendientes.length > 0) {
      msg += `<b>✅ TAREAS (${pendientes.length}):</b>\n`;
      const altas = pendientes.filter(t => (t.prioridad || '') === 'ALTA');
      const otras = pendientes.filter(t => (t.prioridad || '') !== 'ALTA');
      [...altas, ...otras].slice(0, 12).forEach(t => {
        const em = t.prioridad === 'ALTA' ? '🔴' : t.prioridad === 'MEDIA' ? '🟡' : '⚪';
        msg += `${em} ${t.tarea}${t.proyecto ? ` [${t.proyecto}]` : ''}\n`;
      });
      if (pendientes.length > 12) msg += `... y ${pendientes.length - 12} más\n`;
    } else {
      msg += `✅ Sin tareas pendientes\n`;
    }

    await tg('personal', msg);
    programarRecordatorios(eventos);
    console.log('[NOTIF] Briefing PERSONAL enviado');
  } catch (err) {
    console.error('[NOTIF PERSONAL]', err.message);
  }
}

module.exports = { enviarReporteOPS, enviarSaldoDropi, enviarSaldosMañana, enviarCierreNoche, enviarBriefingPersonal };
